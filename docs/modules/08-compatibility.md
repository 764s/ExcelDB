...
- 范围:当前/历史 SchemaDescriptor diff、结构生成兼容分级、字段与类型演进、格式三阶段、数据保留、显式 purge/rekey、各操作门禁与交付节律。
...
| 同 field number 改类型、shape、reference 或关键语义 | blocker | 禁止就地重解释;走 §5 新 number 三阶段 |
...
类型、value shape、引用目标/策略、key语义、export语义等可能改变 canonical value或runtime解释的变化,不得在原 field number 上直接换义。标准节律:
...
| convert/build | 无error/blocker,迁移完成,schema匹配 | 失败,不产可发布bytes |
| Runtime Open/Switch/Refresh | source schema hash等于代码期望 | 失败并按M7保留旧source |
...
- 禁止runtime执行workbook迁移或以旧schema candidate替换当前有效source。
- 禁止把aliases/legacy当作永久双事实源;它们必须有清理证据和明确退出阶段。
- 禁止删除已发布 table message/id、清除 retired 复活旧表，或把任一 live/retired table id 分配给新声明。

## 12. 开放决策

1. **schema hash拆分**:M1当前单一`schema_hash`包含名称、类型/shape、export、引用、format canonical+legacy等语义,排除display/comment/aliases。是否新增runtime semantic/layout/codegen hash尚未决定。裁定前所有工具和M7 source门禁继续只使用M1单hash;不得先实现多hash再让入口产生不同兼容结论。
...
3. 新增optional/required、删除reserved/未reserved、类型换义、key结构变化、ownership重叠逐行命中§4分级。
...
9. lint/generate/import/save/normalize/convert/runtime对同一compatibility输入得到一致或更严格门禁,无入口特例。
10. schema hash拆分在未决状态下有防止实现自行放宽的测试/文档断言。
