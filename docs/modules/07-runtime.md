...
- 范围:`IDataSource`、generated `RuntimeSchemaRegistry`、`RuntimeDatabase`、export target、resident/missing、converted bytes、`ChangeSet`、Open/Switch/Refresh 事务、模式边界、引用 provider 与 runtime hot-path no-GC。
...
`IDataSource` 表示同一 canonical schema/data model 针对一个明确 export target 的只读来源。`client`/`server` 是 schema export target,与 §9 的 `RuntimeMode` 正交；前者决定 runtime read surface,后者决定 Excel、watcher、writeback 等宿主能力门禁。具体接口可以分层实现,但必须满足以下语义:

```csharp
public readonly struct ExportTargetId
{
    // M1 定义的 canonical 稳定 target token；物理表示由 API surface 冻结时确定。
}
...
- Open 产出不可见的 candidate snapshot:规范化行值、identity/key 索引、引用与依赖图、source revision/content hash、实际 SchemaHash、实际 ExportTargetId 与诊断。candidate 在 commit 前不得修改当前 source、resident 实例或索引。
- source 必须声明 read/refresh/watch 能力;RuntimeDatabase 不从 source 类型推断写权限。
- `ConvertedBytesDataSource` 是所有模式可用的正式运行时来源。
- `ExcelDataSource` 仅允许 EditorAuthoring，以及经 session bootstrap/API 明确选择的 EditorPlayDebug 与 Development；构造时必须显式绑定一个 ExportTargetId,只投影该 target 的有效表/字段并提供 read/refresh/watch,不提供 workbook writeback。Release 即使显式请求也必须拒绝。
- 同一 ExportTargetId 下,Excel source 与 converted bytes source 的 runtime read surface、默认值、引用和查询结果必须一致；Excel workbook 中存在但未导出到该 target 的表/字段不得从 RuntimeDatabase 或生成访问器读取。
- 自定义 source 只能通过 `IDataSource` 和 canonical descriptor 接入,必须声明并严格投影单一 ExportTargetId,不得改变或扩大该 target 的字段集合,也不得改变 identity、引用、事务或 ChangeSet 语义。
- source 声明及 candidate 实际读到的 schema hash 必须相同,并且都必须等于 Open 显式传入、随后由 session 固定的 generated `RuntimeSchemaRegistry.ExpectedSchemaHash`;source 声明 ExportTargetId、candidate 实际 ExportTargetId 与 registry `ExpectedExportTarget` 也必须三者相等。格式版本、hash、target 或完整性任一校验失败时 Open 失败,作为 Switch/Refresh candidate 时保留旧 source。禁止从 source 自身、bytes header、manifest、Project、cache 或“最近一次成功值”反推代码期望的 hash 或 target。
- source 中任一对其 ExportTargetId 有效的 runtime-exported 行存在 M5§9.2 所述尚未固化、空缺或仅会话内生成的 pending identity,一律产生 `identity.pending` blocker。Open 必须保持 closed,Switch/Refresh 必须保留旧 source/object graph/index;RuntimeDatabase 不得分配、修复、容忍或降级为 warning,任何模式和 opt-in 都不能绕过。identity 只能先在 authoring 侧固化并重新生成/打开 source。
...
public abstract class RuntimeSchemaRegistry
{
    ...
    public abstract ExportTargetId ExpectedExportTarget { get; }
...
public static class RuntimeDatabase
{
    ...
    public static ExportTargetId exportTarget { get; }
...
2. **Validate**:校验 registry 非空且生成绑定自洽,source 声明 hash = candidate 实际 hash = `registry.ExpectedSchemaHash`,source 声明 target = candidate 实际 target = `registry.ExpectedExportTarget`,并校验 source 格式、完整性、无 pending identity、identity/key 唯一性、该 target 的硬引用与 runtime-exported 数据。任一 blocker 中止;hash/target 比较不得用 source 自洽代替 registry 比较。
...
- Open 的 registry/expected hash/expected target 只在成功 Commit 时成为 session 状态;失败时保持 closed 且不缓存为后续 expected 值。Switch/Refresh 失败不得改变当前 registry、hash 或 target。
...
- `Open`:从 closed 建立首个 session;初始化分配许可;成功时同时固定 generated registry、expected hash、expected target 与初始 source。
- `SwitchDataSource`:沿用 session 固定 registry 验证并打开新 source,再与 resident 全量 join;成功才交换 source。同一 target 的 Excel 与 bytes 双向语义对称,但都不得携带不同 schema hash 或 ExportTargetId；换 target 必须 `Close` 后重新 `Open`。
- `Refresh`:沿用 session 固定 registry,只刷新当前 source;优先用 source fingerprint 与字段级 diff 收窄工作量,但结果必须与全量 candidate 等价。刷新后 source hash 或 ExportTargetId 变化即 blocker,不是动态 schema/target migration。
...
- converted bytes 是 immutable runtime 数据产品,不是 workbook 的压缩副本;每个文件只对应一个 ExportTargetId,只包含对该 target 有效的 runtime-exported 表/字段及其必要运行时闭包。其他 target 或 editor-only 数据不得进入表目录、string table、数据区、索引、引用或运行时元数据。
- 文件至少携带 magic/format/schema_hash/export_target_id、确定序表目录、去重 UTF-8 string table、定长/变长数据区、key/guid 索引、已解析内部引用与完整性 hash。
...
- 相同 descriptor + 相同 ExportTargetId + 相同 target-exported canonical data 必须产 byte-for-byte deterministic 输出;不得受当前时间、机器路径、区域设置或字典遍历顺序影响。
- 打开时校验 magic/format/schema_hash/export_target_id/完整性,并把 bytes header 与 candidate 的 hash/target 分别和 Open 传入或 session 已固定的 generated registry expected hash/target 比较;任一失败拒绝 candidate。伴生 manifest 记录 SchemaHash、ExportTargetId、该 target 的源内容 hash、导出表/行数与工具版本；manifest 的 target 必须与同行 bytes header 相等,但 manifest 不能充当 expected hash/target 或 bytes 实读身份的权威来源。
...
- client/server 等 ExportTargetId 不属于本矩阵,不能由模式、宿主类型或 source kind 推断；同一模式可在不同 session 显式打开不同 target,但单个 session 仍固定一个 target。
- 四种模式都必须显式绑定 generated registry并执行同一 expected/source hash、expected/source target 与 pending identity 硬门禁;Editor/Development 的调试能力不提供放宽入口。
...
2. 同 identity、同 ExportTargetId 下重复加载、Refresh、Excel↔bytes Switch 均保持 canonical 实例;删除 → Missing,再出现 → 复活;不误绑新行。对 client/server 各生成一对 Excel/bytes candidate,断言同 target read surface/值/引用一致且不暴露另一 target 字段。
...
6. 四模式逐格门禁；EditorPlayDebug/Development 分别覆盖显式选择 target-specific bytes、显式选择同 target Excel、显式启停热载及其 Diagnostic，且 Project/EditorPrefs/上次会话均不能改变 source/target 选择；Release 对 Excel/watcher 请求失败并保持无 Excel/watcher/writeback/dynamic migration 路径。client/server 不得出现在 RuntimeMode 枚举或模式分支中。
7. no-GC 清单在 CoreCLR 与 Unity 达到水位线后通过;真实变更分配与水位线增长可归因。
8. Unity/Localized/custom provider 缺失、成功、热路径预绑定与 Core 无宿主依赖各一组测试。
9. registry/hash/target 硬门禁:API surface 不存在可省略 generated registry 或只从 source/bytes header/manifest 推导 expected hash/target 的 Open 路径;null/无效 registry 保持 closed。分别用 Excel/bytes/custom source 覆盖“声明 hash 与实际 hash 不同”“source hash 与 `ExpectedSchemaHash` 不同”“声明 target 与实际 target 不同”“source target 与 `ExpectedExportTarget` 不同”,均拒绝且 Diagnostic 同时含 expected/声明/实际 hash 与 target。成功 Open 后,同 `(hash,target)` Switch/Refresh 可成功;任一不同必须保留旧状态。构造同行 manifest target 与 bytes header 不同,必须在产物校验中失败且不得以 manifest 覆盖 header。只有 Close 后以新 registry/source 重新 Open 才能切换 schema 或 target。
