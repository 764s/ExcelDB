...
| 工件 | 进版本控制 | 说明 |
| --- | --- | --- |
| `*.proto`、`ExcelDb.Project.json` | 是 | 程序拥有;proto 是结构事实源(M1§1),可由 `table create/edit` 向导维护,Project 是唯一项目配置源(M4§3.2) |
| 生成代码(codegen 产物 .cs) | 是 | schema 的强类型投影,不是数据文件;Unity/宿主直接消费源码;新鲜度由 CI 门禁保证(WF7);手改无效(M1§1) |
...
| bytes + manifest(P§7.4) | 否 | 与生成 C# 类型及其 SchemaHash 对应的运行时数据;可重建,CI/构建重新 convert,本地按需 |
...
xlsx 数据 <──用户 Excel 直编                         <──SaveAssets 补丁写回── 编辑器编辑(dirty,内存态)
   ...
   └──data prepare──> check/导入校验(P§6.1) ──convert──> bytes+manifest

[宿主集成边界] 生成 C# 中的 RuntimeSchemaRegistry + source + options ──Open──> 运行时 ⇄ 切源/热载(P§7.5/7.6)
```
...
3. 进入 `table create`:用户选择具体 xlsx 路径并给出表名、key 与首批字段,向导按 M1 规则建议并要求确认数字身份。确认后维护 `.proto` 这一结构事实源,并在同一操作中完成 lint/codegen 与 workbook 结构生成,产出 `proto + C# 类型投影 + xlsx` 三件套;路径不存在时直接创建含 metadata、行 1-3、下拉、`__exceldb_keys` 与伴随列的 workbook(P§5.1/5.2)。任一环节 blocker 均不得只留下其中一部分。
...
5. 对已填写 workbook 显式执行 `data prepare`,把 `pending-new` 的候选 RowGuid 按已确认计划固化;随后执行只读 `check`,通过后执行 `convert`,产出与生成 C# `RuntimeSchemaRegistry.ExpectedSchemaHash` 对应的 bytes + manifest。至此单文件工具的首次闭环完成,提交应入库的 Project/proto/生成代码/xlsx 形成首次基线;宿主编译生成 C# 并执行 Runtime `Open` 属于独立集成边界,不是主引导的完成条件。

失败分支:`init` 被拒绝或无法完整提交 → 目录仍是未初始化态,修复权限/路径后重跑;`table create` 的 schema、codegen 或 workbook 计划出现 blocker → 三件套零替换,修改输入后重跑;`data prepare` 计划 stale → workbook 零写入并重新扫描/确认;`check`/`convert` 失败 → 保留事实源与旧数据工件,按报告修 Excel 后从 `data prepare` 或 `check` 继续。

## 4. WF2 表结构演进(`table edit`,程序主导)

1. 启动 `table edit`,选择已有表并声明新增、重命名、删除或格式变化。向导读取当前 proto/descriptor 与 workbook,把用户意图写成 proto 变更:新增字段用新 number;改名保 number(身份不变即兼容,M1§1);字段删除与字段语义变更走 M8§5 三阶段;已发布表的删除请求必须转换为 M8§3.1 的显式退役,保留原 message/id 并设置 `TableOpts.retired = true`;格式变更走 M1§2 三阶段。向导维护 proto 不改变它的事实源地位,C# 与 Excel 表头都只是该结构的投影。
...
1. 进 Play 时由宿主把当次 source、当前生成代码提供的 `RuntimeSchemaRegistry` 与 `RuntimeBootstrapOptions` 三者显式传给 M7;bytes 模式先做陈旧检测——manifest 内容 hash 对当前 xlsx(M7)不符 → 提示 convert 后进 / 带旧数据进 / 取消。Project、EditorPrefs、source/manifest 与上次会话都不得提供隐式默认或反推 registry expected hash。
2. 当次显式选择 Excel 源并启用 hot reload:改 Excel 保存 → 运行时热载(P§7.5),Game 视图即时生效;Play 中 inspector 改数走 editor API 写回(核 10)→ SaveAssets 落盘 → 同一热载环。两条改数路径的汇合点 = xlsx 文件;编辑器与运行时不共享草稿。
3. 调好 → Convert → SwitchDataSource(bytes) 复验出包数据(M7)。
...
1. CI 门禁语义由本文规定,工具配方由 M4§5 投影:生成代码新鲜度、schema/workbook 一致性、数据校验、convert 与评审工件均必须通过;schema.drift 在 CI check 判 error。
2. 构建:`IPreprocessBuildWithReport` 钩子 convert → `StreamingAssets/ExcelDb/`(P§11);manifest 随 bytes 同行。
3. Release 运行:宿主编译当前生成 C#,把其中的 `RuntimeSchemaRegistry`、`ConvertedBytesDataSource` 与本次 `RuntimeBootstrapOptions` 显式传给 `Open`;无热载与写回(P§7.8)。source 声明、candidate 实读与 registry expected 三个 SchemaHash 必须相等。
4. 线上数据补丁 = 重新 `data prepare/check/convert` + 宿主资源系统分发新 bytes + 使用同一 generated registry 显式 Open(P§7.8);bytes 的 schema_hash 必须等于包内代码的 SchemaHash(P§7.4 校验拒绝)——补丁窗口 = 纯调数,结构变更必须随包发布。

## 10. 失败与恢复总表

| # | 失败点(所在 WF) | 表现 | 恢复动作 |
| --- | --- | --- | --- |
| 1 | lint/兼容 blocker(WF1/WF2) | `table create/edit` 计划拒绝,proto/C#/xlsx 零替换(M1§5) | 在向导中修结构意图;需要高级能力时修 proto 后重跑计划 |
...
| 11 | convert error(WF1/WF7) | 不产出新的 bytes+manifest,旧数据工件不被替换;退出码门禁(P§10) | 按报告修 Excel 后从 check 重跑 |
...
1. 空目录首次闭环:测试目录只有当前平台单文件 exe → 无参数运行 → 完成“Project 初始化 → 创建或调整 Excel 表结构并生成 C# → 填写 Excel 并生成对应数据”三阶段。断言 Project/schemaDir 及约定目录可自洽,不依赖预存程序集、宿主工程、SDK、PATH 工具或网络。`table create` 指定不存在的 xlsx 与最小 ASSET 表 → 一次得到 proto、生成 C# 与 workbook,三者 SchemaHash/数字身份一致且 metadata/伴随列/下拉齐备;填写首行 → data prepare → check → convert,断言仅 `__guid` 被计划写入、业务值不变,bytes+manifest 的 schema_hash 等于生成 registry 的 ExpectedSchemaHash。分别在 init、lint、codegen、xlsx 提交点注入失败,断言无半初始化 Project 且三件套零局部替换;宿主编译 C# 后的 Runtime Open 冒烟归独立集成测试。
2. 演进剧本:`table edit` 对加字段 / rename(number 不变)/ 删字段三例先只看计划,断言零写入;apply 后 proto/C#/xlsx 同步且数据保留(P§12-6 复用)。构造"proto 新、xlsx 旧"仓库态 → CI check 以 schema.drift error 红。
...
7. Play Mode 剧本:宿主显式传入 source + generated registry + options,断言 source 声明/实读 hash 与 registry expected hash 三者相等后 bytes 进 Play(陈旧检测提示一例)→ 切 Excel 源 → 热载 5 事件(P§12-7 复用)→ convert → 切回 bytes 值等价;缺 registry 或任一 hash 不符均拒绝且不反推。
...
- M4 已同步 self-contained 单文件交付、Project 自举配置、三阶段 G1 与 `init`/`table create`/`table edit`/`data prepare`;具体实现仍为 Not implemented,不因文档闭环提升交付状态。
- M1 已同步“向导可事务性维护 proto”入口,且 proto 的结构事实源地位不变;Excel 与生成 C# 仍只是数据事实源/结构投影,不得反向定义 schema。
- 历史裁剪 ADR 第 19 条曾记录评审/对账工件恢复为单一 `diff` 命令；该记录现已归档,当前工具边界只以 M4 为准。
- poc 无工作流对应物;WF 剧本的可执行化随 M6(导入与编辑)与 M7(运行时/adapter)的实现推进。

## 13. 跨模块边界与开放决策

- proto 数字身份、向导可执行的 schema patch 与 codegen 原子提交机制 → M1;C# 始终只是类型投影。
