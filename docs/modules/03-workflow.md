...
| `ExcelDb.Workspace.json` | 是 | 程序拥有;工作空间编排配置(构建目标/生成目录/基线分支/脚本指针,M4§3.2);缺失时门面引导创建,子命令与 Unity 不受影响 |
