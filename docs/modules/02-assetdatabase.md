...
状态:本文是逐模块重推演的第 2 篇,以使用样例锁定 `ExcelDbEditor.AssetDatabase` 的对外契约,取代 `exceldb-lean-plan.md` §6.7 的签名清单。对齐基准 = Unity 2022.3 `UnityEditor.AssetDatabase`,像素级:成员名、参数名、返回类型、重载集与调用形态逐字对照;语义与 Unity 相同的成员不复述文档(P§0 写法规则),偏差全部登记在 §8。模块编号自本篇顺延,并经第 3 篇(工作流,`03-workflow.md`,2026-07-10)再顺延:M1 文中预派的"模块 2(workbook 与身份)/模块 3(导入与编辑)/模块 4(运行时)/模块 5(兼容)"与本文正文所写"模块 3/4/5/6"统一改读"模块 4/5/6/7"。后续模块引用本文记作 M2§x;精简计划记作 P§x,模块 1 记作 M1§x。
