...
M1-M4 中仍存在的 `P§x` 是旧精简计划的稳定迁移别名，不再指向归档文件。读取时只按下表解析；新规则必须直接引用 owner，既有别名可在不改变语义时机械替换。
...
- 既有 Schema PoC 已删除；当前纯 C# 正式实现已按 Project v2 契约完成 M1/M3/M4/M8 重验，PoC 不参与产品解释。
